function [u,errR,snrR,ii,Rfactor]=Ptycho_RGD_f(Y,x,Params,cmapidx,amask)
%blind PtychoPR
...
%u = u0;
u0=Params.init;
Image_recovered = tensor_f([Nx*Ny,Nx*Ny],1); % 不知道从0开始好不好。 初始化问题？ 图片都是从全0开始初始化的吗？
ker = fspecial('gaussian', [64 64], 1);
ker_true = fspecial('gaussian', [64 64], 2);
Image_recovered = Image_recovered.initialize(ker,u0);
%Image_recovered_init = Image_recovered;
eta = Params.eta;
measurement = Y.^2;

for ii=1:itmax
    
    v = reshape(Image_recovered.U{1},[Nx,Ny]);
    w = reshape(Image_recovered.U{2},[Nx,Ny]);
    y0 = forward(Image_recovered.A,v,w,amask,cmapidx);
    Grad_F = Image_recovered.get_proj_grad(cmapidx, y0 ,measurement,amask);
    %disp(sum(Grad_F.U{1}));
    Image_recovered = Image_recovered.retraction(Grad_F, eta);
    u = reshape(Image_recovered.U{1},[Nx,Nx]);

 
    %% Output
    %E=abs(A(u))-Y;
    errR(ii)= norm(v(:)-x(:),'fro')/norm(x,'fro');
...
function phi = forward(cof,v,w,amask,cmapidx) %target w is conj(v)
    v_stack = v(cmapidx);
    v_stack = v_stack.* conj(amask);
    phiv = myfft2(v_stack);
    w_stack = w(cmapidx);
    w_stack = conj(w_stack .* amask);
