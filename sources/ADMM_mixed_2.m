function [u,errR,snrR,ii,masks, Rfactor]=ADMM_mixed_2(Y,x,Params,cmapidx,amask,modes)
%blind PtychoPR
...
for ii=1:itmax
    ...
    %% step III
    %tmp=A(u)-Lambda/beta;
    z1=zz-Lambda; % A - lambda
    %tmpflag=abs(tmp)~=0;
    %    z=(Y+beta*abs(tmp))/(1+beta).*(sign(tmp).*tmpflag)+(1-tmpflag).*z0;
    %z0=(Y+beta*abs(z0))/(1+beta).*sign(z0);
    amp = sqrt(sum( abs(z1).^2,4)); % 64*64*frames amplitute for updation
    for k=1:modes
        z(:,:,:,k) =  ( (Y ./ amp).*z1(:,:,:,k) +  beta*z1(:,:,:,k) ) / (1+beta); %???
    end
