...
%             con_f = conj(con_f);
            ...
            con_f = conj(obj.myconv(conj(y0_s),double(con_ker)));   %不知道conv这些对前两个维度操作会怎么样。保险还是分开写
            ...
            %In this case, parfor is much quicker than for 
            %and 52 cores are much quicker than fewer cores
            % disp('for')
            % for j = 1:J
            %     y0_I(:,:,j) = imfilter(y0_s(:,:,j),double(product(:,:,j)),'circular');
            %     %y0_I(:,:,j) = obj.myconv(y0_s(:,:,j),double(product(:,:,j)));
            % end
            % toc
