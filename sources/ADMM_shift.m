function [u, errR,snrR,ii,masks,cor]=ADMM_shift(Y,x,Params,cmapidx,amask,modes)
%blind PtychoPR
...
% inimask   
    ...
    shifts = [0 0;0 30;0 -30;30 0;-30 0;30 30;30 -30;-30 30;-30 -30];
    masks = mask(:,:,ones(modes,1));
    %maskstacks = mask(:,:,ones(nframes,1),ones(modes,1));
    lambda(1,1) = 1;
    for r = 1:modes % randomly genererateb
        disturbed = circshift(mask,[shifts(r,1),shifts(r,2)]) * lambda(r);
        masks(:,:,r) = disturbed;
    end
    maskstacks = permute(masks(:,:,:,ones(nframes,1)),[1 2 4 3]);
...
u = u0;
ustack = u(cmapidx);
ustacks = ustack(:,:,:,ones(modes,1));
ustacks_lambda = ustack(:,:,:,ones(modes,1)); % *lambda
%ini z
...
for ii=1:itmax
    ...
    if flagBlind==1
       %% update mask
        ...
        for k=1:modes
            ustacks(:,:,:,k) = circshift(ustacks_d4(:,:,:,k),[-shifts(k,1) -shifts(k,2)]);
            ustacks_lambda(:,:,:,k) = lambda(k) * ustacks(:,:,:,k);
        end
            
        
        masks0=mask;
        maskAV=squeeze(sum(abs(ustacks_lambda).^2,[3,4])); %squeeze?
...
%                 ./(maskAV+epsM+b_ratio);
%         else
        mask=( squeeze(sum( conj(ustacks_lambda).*Finv_z0,[3,4])) + epsM*masks0 )./(maskAV+epsM);
        mask = project(mask,famask,tamask);
        
        %% update lambda
        aAV   = squeeze(sum((abs(ustacks).^2) .* (abs(mask).^2),[1,2,3]));
       
        %lambda = squeeze(sum( real(conj(Finv_z0).*(ustacks.*mask)),[1,2,3]))./ aAV; % ? squeeze?
        %lambda = max(lambda,0);
        lambda = squeeze(sum( Finv_z0.*conj(ustacks.*mask),[1,2,3]))./ aAV;
        abs(lambda)
%         %% update shifts
...
%         shifts(2:end,:) = floor(shifts_fr2);
        ...
        for r = 1:modes % randomly genererateb
            mask_r = circshift(mask,[shifts(r,1),shifts(r,2)]) * lambda(r);
            masks(:,:,r) = mask_r;
        end
