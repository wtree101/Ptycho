function [u,con_ker,errR,snrR,snrR_ker,ii,Rfactor]=Ptycho_RGD_GPU(Y,x,Params,cmapidx,amask)
%blind PtychoPR
...
u0=Params.init;
Image_recovered = tensor_GPU([px*py,Nx*Ny,Nx*Ny],1,tensor_type); % 不知道从0开始好不好。 初始化问题？ 图片都是从全0开始初始化的吗？
ker = fspecial('gaussian', [64 64], 5);
ker_true = fspecial('gaussian', [64 64], 2);

load('/home/twubi/demoPtycho_2/outcome/0524/result_RGD_blur210000iterstacks_regular_dist8_blur2_circ_new.mat')
ker = result.ker;
u0 = result.u;
...
eta = Params.eta;  %constant, will go to GPU if needed
measurement = Y.^2;
...
for ii=1:itmax
    ...
    %y0 = forward(Image_recovered.A,con_ker,v,w,amask,cmapidx);
      

    Grad_F = Image_recovered.get_proj_grad(cmapidx, conj(FV),conj(FW),conj(FVFW),y0 ,measurement,normalized_mask,Qoverlap);
    %disp(sum(Grad_F.U{1}));
...
if flagGPU==1
    % u,con_ker,errR,snrR,snrR_ker,ii,Rfactor
    u=gather(u);
    mask=gather(mask);
    AtA=gather(AtA);
    errR=gather(errR);
    snrR=gather(snrR);
    con_ker = gather(con_ker);
    snrR_ker = gather(snrR_ker);
    Rfactor = gather(Rfactor);
end
%whos
